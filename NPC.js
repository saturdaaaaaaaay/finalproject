...
class QuestNPC extends NPC
{
  ...
  receive()
  {
    ...
    for (i = 0; i < inventory.length; i++)
    {
      ...
      if (currentItem.name == this.item.name)
      {
        this.quest.changeState(QUEST_COMPLETE);
      }
    }
    /*
    console.log("recipient: " + this.name);
    //console.log("item: " + item.name);
    console.log("correct item: " + this.quest.questItem.name);
    if (item == this.quest.questItem)
    {
      this.quest.changeState(QUEST_COMPLETE);
